<div class="widget-sub">
    <div class="gia-xe-menu">
        ...
        @if (isset($productsPossionHot))
            ...
            @foreach($productsPossionHot as $product)
                ...
                @php
                    ...
                    if (isset($product->images))
                    {
                        ...
                        if (!empty($list_image_product)) {
                            $image = pare_url_file(array_pop($list_image_product)['pi_name']);
                        }
                        
                    }
                   $product_slug_hot  = $product->pro_slug ? $product->pro_slug : str_slug_fix($product->pro_name);
                @endphp

                <div class="text-center product-possion-item" style="box-shadow: 0 5px 20px rgba(0,0,0,.05);margin-bottom: 20px;padding: 10px 0">
                    <a href="{{ route('get.product.detail',[$product_slug_hot,$product->id]) }}" style="display: flex;justify-content: center" class="clearfix" title="{{ $product->pro_name }}">
                    ...
                    <h2><a href="{{ route('get.product.detail',[$product_slug_hot,$product->id]) }}" title="{{ $product->pro_name }}"  style="font-size: 14px;text-transform: capitalize;color: #333;font-weight: 500">{{ $product->pro_name }}</a></h2>
