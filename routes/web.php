...
Route::group(['namespace' => 'Frontend', 'middleware' => ['web','visitor']], function () {
    ...
    // menu
    Route::get('/chuyên-mục/{slug}-{id}','MenuController@action')->name('get.menu.action');
    ...
    Route::get('/sản-phẩm/{slug}-{id}','ProductController@getProducDetail')->name('get.product.detail');
    // Route::post('/sản-phẩm/insert_rating','CommentController@addComment');
    //chi tiet bai viet
    Route::get('bài-viết/{slug}-{id}','ArticleController@getArticleDetail')->name('get.article.detail');

    Route::post('/bai-viet/tang-view','ArticleController@tangView')->name('post.tangView');

    Route::get('/tìm-kiếm/{slug?}','SearchController@search')->name('get.search');
    /// comment
